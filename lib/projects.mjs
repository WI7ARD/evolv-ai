...
import { canonicalRoot } from "./canonical-path.mjs";
...
export class ProjectService {
  ...
  async initialize() {
    ...
    if (this.legacyWorkspaceRoot && !this.db.prepare("SELECT 1 FROM project_grants WHERE project_id=?").get(project.id)) {
      const canonical = await canonicalRoot(this.legacyWorkspaceRoot);
  ...
  async connectGrant(projectId, grant) {
    ...
    // The host claims the folder; Evolv still stores it in its own canonical
    // form, because that is the form rootFor will verify against later.
    const canonical = await canonicalRoot(this.host.claimRoot(this.profileId, selected));
  ...
  async rootFor(projectId) {
    ...
    let canonical;
    // Must use the same canonical form the grant was saved with, or an
    // unchanged folder reads as a changed one.
    try { canonical = await canonicalRoot(grant.rootPath); } catch { throw fail("The project folder is unavailable. Reconnect it.", 409, "PROJECT_FOLDER_MISSING"); }
