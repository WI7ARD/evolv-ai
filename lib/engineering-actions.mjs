...
export class EngineeringActionService {
  constructor({ database, workspaceRoot, fetchImpl = globalThis.fetch, lookup = dns.lookup, approvalService = null, projectService = null, sandboxService = null }) {
  ...
  // The configured workspace root is Evolv's own source directory. That is a
  // reasonable default only where there is no project service to ask — an
  // embedded or test construction. Whenever projects exist, falling back to it
  // would silently hand a tool access to the application's own source and
  // security files, so refuse instead of quietly widening scope.
  #fallbackRoot() {
    if (this.projectService) {
      throw actionError("Select a project before using filesystem actions.", 409, "PROJECT_REQUIRED");
    }
    return this.root;
  }
  ...
  async actionRoot(input = {}) {
    ...
    return input.projectRoot ? path.resolve(input.projectRoot) : this.#fallbackRoot();
  }

  async safePath(input, { allowMissing = false, projectRoot = "" } = {}) {
    const allowedRoot = projectRoot ? path.resolve(projectRoot) : this.#fallbackRoot();
  ...
  async runCommand(command, timeoutMs, projectRoot = null) {
    const cwd = projectRoot ? path.resolve(projectRoot) : this.#fallbackRoot();
  ...
  async decide(runId, decision) {
    ...
    try {
      ...
      } else if (action.kind === "sandbox-promotion") {
        if (!this.sandboxService) throw actionError("The sandbox is unavailable in this build.", 409, "CAPABILITY_UNAVAILABLE");
        // The sandbox re-verifies every target against the real workspace and
        // refuses the whole set if anything moved, so approval cannot apply
        // work built on a stale copy.
        const promoted = await this.sandboxService.promote(action.payload.sessionId);
        result = { sessionId: action.payload.sessionId, applied: promoted.applied, files: promoted.applied.length };
