...
export function toLegacyChunk(event) {
  ...
  switch (event.type) {
    ...
    // provider.state describes the assistant turn rather than one call:
    // reasoning items with their signatures, and the response item ids a
    // provider wants back on the next request. Both adapters read these from
    // message.provider_state, so they have to be collected and stored, not only
    // the tool calls.
    case "provider.state":
      return { providerState: event.state };
    case "fallback":
    default:
      return null;
  }
