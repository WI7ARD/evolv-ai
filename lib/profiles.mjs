...
export function createProfileManager({
  ...
  async function initialize(userId) {
    ...
    // Built before the goal runner because the runner reads a pack's
    // specialists when it plans a goal that belongs to that pack.
    ...
    const goalRunner = new GoalRunnerService({
      database, agentRuntime, toolRegistry, providerService, vaultService, projectService, marketplace
    });
