...
export class AgentRuntime {
  ...
  createGoalRun({ conversationId, projectId, objective, successCriteria = [], providerId = "", modelId = "", request = {}, budgets = {}, plan, availableTools = [] }) {
    const runId = crypto.randomUUID();
    ...
    this.db.transaction(() => {
      this.db.prepare(`INSERT INTO agent_runs(id,conversation_id,objective,state,executor,provider_id,model_id,request_json,resume_state,created_at,updated_at)
        ...
        JSON.stringify({ ...request, projectId, successCriteria: criteria }), createdAt, createdAt
      );
      ...
      this.db.prepare(`INSERT INTO agent_goals(id,run_id,project_id,pack_id,success_criteria_json,status,created_at,updated_at)
        VALUES(?,?,?,NULL,?,'planned',?,?)`).run(goalId, runId, projectId || null, JSON.stringify(criteria), createdAt, createdAt);
  ...
  get(id) {
    ...
    row.goal = this.db.prepare(`SELECT id,project_id AS projectId,success_criteria_json AS successCriteriaJson,status,created_at AS createdAt,updated_at AS updatedAt FROM agent_goals WHERE run_id=?`).get(id) || null;
