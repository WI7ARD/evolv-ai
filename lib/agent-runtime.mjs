...
export class AgentRuntime {
  ...
  updateRoute(runId, { providerId = "", modelId = "", reason = "model fallback" } = {}) {
    const run = this.get(runId);
    if (!run) throw runtimeError("Agent run not found.", "RUN_NOT_FOUND", 404);
    const changedAt = timestamp();
    this.db.prepare("UPDATE agent_runs SET provider_id=?,model_id=?,updated_at=? WHERE id=?")
      .run(String(providerId).slice(0, 50), String(modelId).slice(0, 300), changedAt, runId);
    this.#appendEvent(runId, null, "routing.fallback", {
      providerId: String(providerId).slice(0, 50),
      modelId: String(modelId).slice(0, 300),
