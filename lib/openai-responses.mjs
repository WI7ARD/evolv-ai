...
// Which effort to ask for, or nothing at all.
//
// Two ways this used to send something OpenAI refuses. Thinking turned off
// produced `reasoning: { effort: "none" }` — a key present, saying "none",
// where the request should simply not carry one; a model that does not reason
// rejects the parameter outright rather than reading it as "do not". And
// "xhigh" was passed straight through, which is a level from another vendor's
// vocabulary and not one of OpenAI's.
//
// So: an empty string means omit the key, and only the three levels OpenAI
// documents are ever sent. Anything unrecognised omits rather than guesses,
// because a request without the parameter still works and a request with a
// wrong value does not.
function reasoningEffort(value) {
  if (value === false || value === "none" || value == null || value === "") return "";
...
export async function streamOpenAiResponses({ payload, signal, request, onEvent }) {
  ...
  const items = new Map();
  // Every branch below that produces something the caller can use marks the
  // event understood. If none of them ever fire, the stream is reported rather
  // than silently yielding an empty turn — see lib/stream-witness.mjs.
  const witness = createStreamWitness("OpenAI");
  await parseSse(response, (event) => {
    const type = eventType(event);
    witness.saw(type);
    if (type === "response.output_text.delta" && event.delta) { witness.understood(); onEvent(textDelta(String(event.delta))); }
