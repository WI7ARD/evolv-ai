...
function assignAgent(requested, { type, tool }, roster) {
  const use = stepToolUse({ type, tool });
  const chosen = resolveAgent(requested, { step: { type }, roster });
  if (agentMayRunStep(chosen, use)) return chosen.id;
  const fallback = resolveAgent(defaultAgentForStep({ type }), { step: { type }, roster });
...
export function validateGoalPlan(input, { availableTools = [], budgets = GOAL_BUDGET_LIMITS, roster = undefined } = {}) {
  ...
  const steps = input.steps.map((raw, index) => {
    ...
    return {
      ...
      agent: assignAgent(raw.agent, { type, tool }, roster)
    };
  });
