...
import { EVOLV_VERSION } from "./version.mjs";
...
// Packs declare the oldest Evolv they support, and `validateManifest` gates
// every install on it, so this must track the shipped release rather than a
// literal someone has to remember to edit.
// Imported, not re-exported straight through: this module uses the value in
// its own default parameters, and `export ... from` would not bind it locally.
export { EVOLV_VERSION };
...
function pack(definition) {
  ...
  const manifest = {
    ...
    models: { local: ["qwen", "llama", "phi", "mistral"], cloud: ["openai"] },
    permissions,
