...
import { Coupling, exchange, couplingError, COUPLING_SECONDS } from "./coupling.mjs";
...
export class BenchService {
  ...
  constructor({ database, circuitService, physicsService = null }) {
    this.#database = database;
    this.#circuit = circuitService;
    this.#physics = physicsService;
  }

  get coupling() { return this.#coupling; }
  ...
  // A check runs the circuit and then judges it, so it satisfies both stages.
  // Recording only the verdict left the timeline claiming the board had never
  // been run while showing the figures that run produced.
  check(options = {}) {
    const result = this.#circuit.apply("check", options);
    this.recordRun({ ...result, time: result.seconds });
  ...
  // The coupled run: the board and the world, stepping together.
  //
  // One exchange per world step. Between exchanges each simulation runs at its
  // own timestep and sees the other held constant, which is what makes this
  // affordable — the circuit takes thousands of microsecond steps inside each
  // sixteen-millisecond slice, and the world takes one.
  //
  // Both are advanced by the same interval, so simulated time in the two stays
  // together. Without that the readings could not be compared at all: a wheel
  // speed from one clock beside a firmware decision from another is two
  // different experiments.
  runCoupled({ seconds = 1 } = {}) {
    if (!this.#physics) throw couplingError("The physics world is unavailable in this build.", "CAPABILITY_UNAVAILABLE", 503);
    const links = this.#coupling.list();
    if (!links.length) {
      throw couplingError("Nothing is linked yet. Join a circuit motor to something in the world first.", "LINK_NONE");
    }
    const asked = Math.max(COUPLING_SECONDS, Math.min(30, Number(seconds) || 1));
    const intervals = Math.round(asked / COUPLING_SECONDS);
    // One line per fault, not per interval. Keyed on what is wrong rather than
    // on the sentence describing it, because the sentence carries figures that
    // change as a motor spins up — so a set of sentences deduped nothing. The
    // last one wins, which after a spin-up is the steady state.
    const problems = new Map();

    for (let interval = 0; interval < intervals; interval += 1) {
      const crossing = exchange(links, {
        parts: this.#circuit.frame().devices || {},
        bodies: Object.fromEntries(this.#physics.perceive().objects.map((object) => [object.id, object]))
      });
      for (const note of crossing.problems) problems.set(note.key, note.message);
      for (const drive of crossing.drives) {
        try {
          this.#physics.setMotorSpeed(drive.body, drive.radiansPerStep);
        } catch (error) {
          // The world's own message, not a guess at what went wrong. A bare
          // catch here reported "is not a motor" for a speed that was merely
          // out of range — a fabricated diagnosis in place of an accurate one,
          // which is the fault that cost this project a whole incident report.
          problems.set(`refused:${drive.body}`, `${drive.body}: ${error.message}`);
        }
      }
      if (Object.keys(crossing.conditions).length) this.#circuit.apply("conditions", crossing.conditions);
      this.#circuit.apply("run", { seconds: COUPLING_SECONDS });
      this.#physics.step(1);
    }

    const result = {
      ran: true,
      seconds: Number((intervals * COUPLING_SECONDS).toFixed(6)),
      intervals,
      couplingSeconds: COUPLING_SECONDS,
      links: links.length,
      circuit: this.#circuit.perceive(),
      world: this.#physics.perceive(),
      problems: [...problems.values()]
    };
    // A coupled run runs the circuit, so the board records that it did — the
    // same rule an ordinary run and a check follow.
    this.recordRun({ ran: true, time: result.seconds, currents: this.#circuit.frame().currents || {}, findings: [] });
    return result;
  }

  link(input) { return this.#coupling.add(input); }
  unlink(id) { return this.#coupling.remove(id); }
  links() { return this.#coupling.list(); }
