...
import { DEFAULT_EVOLV_MODEL, evolvModelDefinition, isEvolvModel, listEvolvModels } from "./evolv-models.mjs";
...
// Ollama returns the system prompt with whatever whitespace it stored, and a
// prompt is compared for meaning rather than for bytes.
...
export function createEvolvLocalService({ client, clock = () => Date.now(), log = () => {} } = {}) {
  let run = null;
  // Keyed by digest: a model that has not been rebuilt cannot have changed, so
  // the extra request happens once rather than on every status poll.
  const promptCache = new Map();

  // Editing EVOLV_SYSTEM_PROMPT does not change a model Ollama has already
  // built — it baked in the prompt as it read at install time. Without this
  // check, an improved prompt would silently reach only new installs.
  async function isStale(installed, definition) {
    const entry = installed.find((model) => model.name === definition.name);
    if (!entry) return false;
    const key = entry.digest || entry.name;
    if (promptCache.has(key)) return promptCache.get(key);
    try {
      const shown = await client.show(definition.name);
      // No answer is not evidence of staleness; saying "out of date" because a
      // request failed would send someone to rebuild a model that is fine.
      if (!shown || typeof shown.system !== "string") return false;
      const stale = !samePrompt(shown.system, definition.system);
      promptCache.set(key, stale);
      return stale;
    } catch {
      return false;
    }
  ...
  async function status({ preferred = "" } = {}) {
    ...
    // Any build in the catalogue counts, not only evolv:latest. Asking about
    // one name while the setup panel offers another is how someone installs
    // Evolv Local, is told it is not installed, and cannot find the model they
    // were just given.
    const installedModels = reachable ? listEvolvModels().filter((entry) => has(entry.name)) : [];
    // The largest installed is the one to treat as Evolv Local: it is the best
    // answer this machine currently has. The catalogue is ordered smallest
    // first, so the last match is the biggest.
    const active = installedModels.at(-1)
      || evolvModelDefinition(preferred && isEvolvModel(preferred) ? preferred : DEFAULT_EVOLV_MODEL);
    const definition = active;
    const installed = installedModels.length > 0;

    return {
      // True when the model exists but was built from an older system prompt.
      // Rebuilding is cheap — the base model is already on disk.
      evolvModelStale: installed ? await isStale(models, definition) : false,
      ...
      evolvModelInstalled: installed,
      // Every Evolv build present, so the interface can say which one is
      // running rather than assuming the default.
      evolvModelsInstalled: installedModels.map((entry) => entry.name),
  ...
  // A queue rather than one model: Evolv Local is a ladder of builds, and
  // asking someone to run the installer three times — watching each finish
  // before starting the next — is a worse way to spend the same bytes.
  function begin(modelNames) {
    const definitions = modelNames.map((name) => evolvModelDefinition(name));
    const controller = new AbortController();
    const state = {
      phase: "checking",
      model: definitions[0].name,
      base: definitions[0].base,
      // What the whole run covers, so the interface can say "2 of 3" rather
      // than restarting its progress with no explanation.
      queue: definitions.map((entry) => entry.name),
      queueIndex: 0,
    ...
    const done = (async () => {
      ...
      try {
        ...
        definition = entry;
        publish(state, {
          model: definition.name, base: definition.base, queueIndex: index,
          phase: "checking", status: `Checking what is already installed…`, percent: null
        });
        const installed = (await client.listModels()).map((model) => model.name);
        ensureLive();
        // In a queue, a build that is already there is skipped rather than
        // rebuilt. Asked for on its own it is still rebuilt, because that is
        // what the rebuild button does.
        if (definitions.length > 1 && installed.includes(definition.name)) {
          publish(state, { status: `${definition.label} is already installed.`, percent: 100 });
          continue;
        }
        ...
        // The rebuilt model carries the current prompt, so any earlier verdict
        ...
        publish(state, {
          phase: "ready",
          status: definitions.length > 1
            ? `${definitions.length} Evolv Local builds are ready.`
            : `${definitions[0].label} is ready.`,
          percent: 100,
          error: null
        });
      } catch (error) {
    ...
    return { state, controller, done, model: state.queue.join(","), models: state.queue };
  }
  ...
  return {
    ...
    install({ model = DEFAULT_EVOLV_MODEL, models = null } = {}) {
      // One name or several; duplicates collapse, because asking for the same
      // build twice in one queue is a request to download it twice.
      const requested = [...new Set((Array.isArray(models) && models.length ? models : [model]).map(String))];
      for (const name of requested) {
        if (!isEvolvModel(name)) evolvModelDefinition(name); // throws a 404 naming the known models
      }
      const key = requested.join(",");
      if (run && !run.state.finished) {
        if (run.model !== key) {
          throw Object.assign(new Error(`${run.state.model} is already being installed. Wait for it to finish.`), {
