...
export function stageReport(board = {}) {
  ...
  return BOARD_STAGES.map((stage) => {
    ...
    return {
      id: stage.id, label: stage.label,
      state: changed.length ? "stale" : "done",
...
export function boardSummary(board = {}) {
  ...
  const reached = report.filter((stage) => stage.state === "done").at(-1);
  if (stale) {
...
export function compareMeasurement(measurement, verify = null) {
  ...
  if (simulated === null) {
    return {
      ...
      detail: `Measured ${shown}. Nothing in the last check covers ${measurement.subject} ${measurement.measure}, so there is nothing to compare it against.`
    };
  }
  ...
  const ratio = scale > 0 ? Math.abs(difference) / scale : null;
  const agreement = ratio === null ? "unchecked"
    : ratio <= AGREEMENT_CLOSE ? "agrees"
    : ratio <= AGREEMENT_NEAR ? "near"
    : "differs";
  const gap = ratio === null ? "" : ` — ${(ratio * 100).toFixed(0)}% ${difference > 0 ? "above" : "below"}`;
  return {
    ...measurement, simulated, difference, ratio, agreement,
    detail: `Simulated ${formatMeasure(measurement.measure, simulated)}, measured ${shown}${gap}.`
  };
}
