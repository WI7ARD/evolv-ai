...
// Devices paired with this computer for the phone companion. Only a hash of
// each token is kept: a stolen database must not yield a working credential.
// Pairing codes are deliberately absent — they live in memory for minutes and
// writing a short-lived secret to disk only widens where it can leak from.
...
CREATE INDEX IF NOT EXISTS idx_companion_devices_active ON companion_devices(revoked_at, created_at DESC);
`;
...
export const PROFILE_MIGRATIONS = Object.freeze([
  ...
  { version: 17, name: "companion-devices", sql: COMPANION_SCHEMA }
]);
