...
// Saved circuits, alongside saved physics scenes and on the same terms.
//
// This lives in its own migration rather than at the end of the physics one,
// which is where it was first written. Appending it there changed the recorded
// SQL of a migration every existing database had already run, so the ledger's
// integrity check refused to open any profile created before that edit — the
// check working exactly as designed, against a schema change that should never
// have been made in place.
const CIRCUITS_SCHEMA = `
...
// What each cloud turn actually used, from the provider's own report.
//
// Kept as its own rows rather than as a column on a message: a turn can span
// several rounds, a run can span several turns, and the question people ask is
// "what did this month cost", which is a sum over time rather than a lookup on
// one message.
//
// The token counts are exact — they are what the provider said. The cost is an
// estimate from published list prices and is stored in micro-dollars so it sums
// as an integer.
...
CREATE INDEX IF NOT EXISTS idx_token_usage_conversation ON token_usage(conversation_id, created_at DESC);
`;

// A board, and everything that has happened to it.
//
// The saved-circuit table above stored a design and nothing else, so a circuit
// reopened a week later could not say whether it had ever been checked, what
// had been exported from it, or whether anyone had built it. A board is the
// same design plus that history, which is what makes the sandbox a bench rather
// than a drawing tool.
//
// The snapshot stays opaque to SQL, exactly as it is for circuits and physics
// scenes: the simulator understands it and the database does not, which is what
// keeps the simulator memory-only and its tools in the automatic risk tier.
// stages_json is the same bargain for the history — lib/boards.mjs owns its
// meaning, and adding a stage does not need a migration.
//
// Measurements are rows rather than a blob, because they are the one part of a
// board that accumulates over time and gets asked questions across boards:
// "where has the simulation been wrong" is a query, not a document.
const BOARDS_SCHEMA = `
...
CREATE TABLE IF NOT EXISTS board_measurements (
  id TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_board_measurements_board ON board_measurements(board_id, measured_at DESC);

-- Every circuit saved before boards existed becomes a board, keeping its id and
-- its dates. Migrating rather than leaving them behind: a saved circuit is a
-- board that has only reached the design stage, and asking people to re-save
-- their work to see it in the new place is how you lose it.
INSERT OR IGNORE INTO boards(id, project_id, name, intent, snapshot_json, stages_json, part_count, spend_micros, created_at, updated_at)
  SELECT id, NULL, name, '', snapshot_json, '{}', part_count, 0, created_at, updated_at FROM circuits;
`;
...
export const PROFILE_MIGRATIONS = Object.freeze([
  ...
  { version: 18, name: "circuits", sql: CIRCUITS_SCHEMA },
  { version: 19, name: "token-usage", sql: TOKEN_USAGE_SCHEMA },
  { version: 20, name: "boards", sql: BOARDS_SCHEMA }
]);
