...
export const TOOL_RISK_POLICIES = Object.freeze({
  ...
  // Writes that land in Evolv's own record of its own work: a board's name, the
  // sentence saying what it is for, a figure someone read off a meter.
  //
  // Automatic, for the same reason sandbox is — nothing here reaches the user's
  // files, the network, or anything outside the profile database, and every one
  // of them is undone by deleting the row. But not sandbox, because a board is
  // not disposable and the effect field would be lying if it said so. The
  // distinction is worth a class of its own: the automatic tier is a permission
  // surface, and the honest way to widen it is to name what was added.
  record: Object.freeze({ id: "record", automatic: true, approval: "none", effect: "record-write", enabled: true }),
