// Evolv Circuit: a schematic Evolv can build in and look at.
//
...
import { toKicadNetlist, toBomCsv, exportProblems, safeName } from "./circuit/kicad.mjs";
...
export class CircuitService {
  ...
  // Hand the design to something that can make a board.
  //
  // The netlist is what Pcbnew reads to lay a board out, and the bill of
  // materials is what a supplier's upload form wants. Between them they are the
  // door out of the sandbox — everything before this answers questions about a
  // circuit, and this lets you stop asking and build it.
  export({ format = "netlist", name = "" } = {}) {
    const components = this.#list();
    if (!components.length) throw fail("There is nothing to export — the circuit is empty.", "CIRCUIT_EMPTY");
    const title = String(name || "Evolv Circuit").trim().slice(0, 60) || "Evolv Circuit";
    const safe = safeName(title);
    if (format === "bom") {
      return {
        format, filename: `${safe}-bom.csv`, contentType: "text/csv",
        content: toBomCsv(components, billOfMaterials(components))
      };
    }
    if (format !== "netlist") throw fail('format must be "netlist" or "bom".', "CIRCUIT_EXPORT_INVALID");
    return {
      format, filename: `${safe}.net`, contentType: "text/plain",
      content: toKicadNetlist(components, { name: title }),
      // What the export could not answer for, said here as well as thrown, so a
      // model that asks whether a circuit is exportable gets a list rather than
      // having to trigger the failure to find out.
      problems: exportProblems(components)
    };
  }
  ...
  check({ seconds = 0, dt = 0, method = "euler" } = {}) {
    ...
    // A chip that never ran is the reason for every expectation about it, and
    // the summary is the line anyone reads first. "Never switched — it sat
    // between 0% and 0%" is true and points at the LED; the typo on line 2 is
    // what actually needs fixing.
    const broken = result.findings.filter((finding) => String(finding.code || "").startsWith("FIRMWARE_"));
    const summary = broken.length
      ? `${broken[0].message}. Everything after that follows from it. ${summarise(results)}`
      : summarise(results);
    return {
      ran: true,
      ...
      // A check is a run with a verdict on the end of it, so it answers the
      // same question a run does about what the circuit was drawing. Without
      // this the board's timeline said "Simulate: not run yet" directly beside
      // a verdict read off a full transient.
  ...
  apply(action, parameters = {}) {
    switch (action) {
      ...
      case "export": return this.export(parameters);
...
export const CIRCUIT_ACTIONS = Object.freeze(["add", "wire", "adjust", "remove", "clear", "run", "probe", "unprobe", "rewind", "firmware", "conditions", "export", "expect", "unexpect", "check", "restore"]);
