...
export class CircuitService {
  ...
  probe(target) {
    ...
    const kind = this.#components.has(name) ? "part" : "net";
    ...
    this.#probes.set(name, { kind, target: name });
    this.#cache = null;
    return { probes: [...this.#probes.values()] };
  }

  unprobe(target) {
    this.#probes.delete(String(target || "").trim());
    this.#cache = null;
    return { probes: [...this.#probes.values()] };
  }
  ...
  run({ seconds, dt = 0, method = "euler", restart = false } = {}) {
    ...
    this.#atTime = { nets: result.nodes, currents: result.currents };
  ...
  perceive() {
    ...
    const components = this.#list();
    const dc = solveNetlist(components);
    ...
    const result = running
      ? { ...dc, nodes: this.#atTime.nets, currents: this.#atTime.currents, solved: true }
      : dc;
    const view = {
  ...
  apply(action, parameters = {}) {
    switch (action) {
      ...
      case "probe": return this.probe(parameters.target);
...
export const CIRCUIT_ACTIONS = Object.freeze(["add", "wire", "adjust", "remove", "clear", "run", "probe", "unprobe", "rewind", "restore"]);
