...
// How many runs each arm needs before the comparison is willing to say anything
// at all. Eight is not a lot; it is the point below which the arithmetic is
// theatre. Under it the harness reports the numbers and refuses the conclusion.
const MINIMUM_COHORT = 8;
...
// Two-proportion z, pooled. Used for rates — completion, grounding — where each
// run is a yes or a no.
function proportionGap(aHits, aTotal, bHits, bTotal) {
  if (aTotal < 2 || bTotal < 2) return null;
  const pooled = (aHits + bHits) / (aTotal + bTotal);
  const error = Math.sqrt(pooled * (1 - pooled) * ((1 / aTotal) + (1 / bTotal)));
  const delta = (aHits / aTotal) - (bHits / bTotal);
  return { delta, z: error ? delta / error : 0, separated: Boolean(error) && Math.abs(delta / error) >= 1.96 };
}

// Welch on the means, normal approximation. At the sample sizes a person
// actually accumulates running goals by hand this is optimistic — the normal
// tails are thinner than the t tails — which is why `separated` is reported as
// a hint and the verdict never rests on it alone.
...
function summarizeCohort(entries) {
  const finished = entries.filter((item) => item.metrics.completion != null);
  const reliability = entries.map((item) => item.metrics.toolReliability).filter((value) => value != null);
  const ratings = entries.map((item) => item.feedbackRating).filter((value) => value != null);
  const latencies = entries.map((item) => item.metrics.latencyMs).filter((value) => value != null);
  return {
    runs: entries.length,
    finishedRuns: finished.length,
    completed: finished.filter((item) => item.metrics.completion === 1).length,
    completionRate: round(mean(finished.map((item) => item.metrics.completion))),
    grounded: entries.filter((item) => item.metrics.grounding === 1).length,
    groundingRate: entries.length ? round(entries.filter((item) => item.metrics.grounding === 1).length / entries.length) : null,
    toolReliability: round(mean(reliability)),
    toolCalls: round(mean(entries.map((item) => item.metrics.toolCalls || 0)), 2),
    toolFailures: round(mean(entries.map((item) => item.metrics.toolFailures || 0)), 2),
    meanLatencyMs: latencies.length ? Math.round(mean(latencies)) : null,
    meanTokens: round(mean(entries.map((item) => item.metrics.tokens || 0)), 1),
    meanCostUnits: round(mean(entries.map((item) => item.metrics.estimatedCostUnits || 0)), 4),
    ratedRuns: ratings.length,
    meanRating: round(mean(ratings), 2)
  };
}
...
export class EvolutionService {
  ...
  evaluateRun(runId, { messageId = "" } = {}) {
    ...
    // Which arm of the specialist comparison this run belongs to, taken from
    // what the run recorded at creation rather than from the setting as it
    // stands now — a setting flipped since then would silently relabel history.
    // Runs that predate the control arm carry no flag at all and are counted in
    // neither cohort, because they were not an experiment.
    const specialists = typeof run.request?.specialists === "boolean" ? run.request.specialists : null;
    const agents = [...new Set((run.events || [])
      .filter((event) => event.type === "agent.assigned")
      .map((event) => String(event.payload?.agent || "")).filter(Boolean))];
    const evidence = {
      ...
      specialists, agents, stepCount: run.steps.length
    };
    const stamp = now();
  ...
  // Did specialists help?
  //
  // Every goal ran with specialists from the day they were added, so there was
  // nothing to compare against and the claim behind the feature — that a model
  // told which job it is doing does it better — went untested. Turning them off
  // produces a control arm; this reads the two arms back out of the evaluations
  // already being recorded and says what the numbers do and do not support.
  //
  // It is deliberately hard to get a "yes" out of this. Both arms need runs,
  // both need enough of them, and a rate has to be far enough apart to survive
  // its own standard error. Anything short of that comes back as no verdict,
  // with the numbers attached so they can be looked at anyway.
  compareSpecialists({ limit = 500 } = {}) {
    const rows = this.db.prepare("SELECT * FROM run_evaluations ORDER BY created_at DESC LIMIT ?")
      .all(Math.max(1, Math.min(2000, Number(limit) || 500))).map(mapEvaluation);
    const labelled = rows.filter((row) => typeof row.evidence?.specialists === "boolean");
    const withSpecialists = labelled.filter((row) => row.evidence.specialists);
    const control = labelled.filter((row) => !row.evidence.specialists);
    const cohorts = { specialists: summarizeCohort(withSpecialists), control: summarizeCohort(control) };
    const enough = cohorts.specialists.runs >= MINIMUM_COHORT && cohorts.control.runs >= MINIMUM_COHORT;
    const comparison = {
      completionRate: proportionGap(
        cohorts.specialists.completed, cohorts.specialists.finishedRuns,
        cohorts.control.completed, cohorts.control.finishedRuns),
      groundingRate: proportionGap(
        cohorts.specialists.grounded, cohorts.specialists.runs,
        cohorts.control.grounded, cohorts.control.runs),
      toolReliability: meanGap(
        withSpecialists.map((row) => row.metrics.toolReliability).filter((value) => value != null),
        control.map((row) => row.metrics.toolReliability).filter((value) => value != null)),
      rating: meanGap(
        withSpecialists.map((row) => row.feedbackRating).filter((value) => value != null),
        control.map((row) => row.feedbackRating).filter((value) => value != null)),
      costUnits: meanGap(
        withSpecialists.map((row) => row.metrics.estimatedCostUnits || 0),
        control.map((row) => row.metrics.estimatedCostUnits || 0))
    };
    const separated = enough
      ? Object.entries(comparison).filter(([, gap]) => gap?.separated).map(([metric]) => metric)
      : [];
    return {
      minimumCohort: MINIMUM_COHORT,
      unlabelledRuns: rows.length - labelled.length,
      cohorts,
      comparison,
      conclusive: enough && separated.length > 0,
      separated,
      method: "Two-proportion z for rates, Welch on the means, normal approximation. Treat a separation as a reason to keep running goals in both arms, not as a result.",
      verdict: this.#specialistVerdict(cohorts, comparison, enough, separated)
    };
  }

  #specialistVerdict(cohorts, comparison, enough, separated) {
    if (!cohorts.specialists.runs && !cohorts.control.runs) {
      return "No goal has finished since the comparison was added. Run some with specialists on, then some with them off.";
    }
    if (!cohorts.control.runs) return "Nothing has run with specialists turned off, so there is nothing to compare against.";
    if (!cohorts.specialists.runs) return "Nothing has run with specialists turned on since the comparison was added.";
    if (!enough) {
      return `Too few runs to conclude anything: ${cohorts.specialists.runs} with specialists against ${cohorts.control.runs} without, and ${MINIMUM_COHORT} of each is the floor. The numbers below are real but they are not evidence yet.`;
    }
    if (!separated.length) {
      return `${cohorts.specialists.runs} runs with specialists against ${cohorts.control.runs} without, and no measure separates from its own noise. On this evidence specialists are neither helping nor hurting.`;
    }
    const direction = separated.map((metric) => {
      const gap = comparison[metric];
      return `${metric} ${gap.delta > 0 ? "higher" : "lower"} with specialists (${gap.delta > 0 ? "+" : ""}${round(gap.delta, 3)})`;
    });
    return `${cohorts.specialists.runs} runs with specialists against ${cohorts.control.runs} without: ${direction.join("; ")}.`;
  }
