...
// The world runs at a fixed 60 steps a second. Named because anything reading
// an angular velocity out of Matter needs it to turn radians-per-step into
// radians-per-second, and a bare 60 in that expression reads as a coincidence.
export const STEPS_PER_SECOND = 60;
export const WORLD_WIDTH = 800;
export const WORLD_HEIGHT = 600;
const WALL_THICKNESS = 60;

// Bumped when the meaning of a saved scene changes. A scene claiming a higher
// number is refused rather than half-understood.
...
export class PhysicsService {
  ...
  #held = null;
  // The scene as it stood before time was first advanced, so Reset can undo a
  // run without rebuilding it by hand.
  #initial = null;
  #restoringId = null;
  ...
  #nextId(shape) {
    // A restore hands the next creator the id it is replacing, so joints saved
    // as "box-3 to circle-7" reconnect to the same two objects.
    if (this.#restoringId) {
      const id = this.#restoringId;
      this.#restoringId = null;
      return this.#claimId(id);
    }
    this.#sequence += 1;
  ...
  // `params` is the argument object that produced this group. Keeping it is
  // what makes a scene rebuildable: body geometry alone cannot reconstruct a
  // ragdoll's joints or a car's axles, so a snapshot records how a thing was
  // made as well as where it currently is.
  #register(bodies, constraints, id, kind, note, material = "default", params = {}) {
    const list = Array.isArray(bodies) ? bodies : [bodies];
    for (const body of list) body.label = id;
    this.#groups.set(id, {
      kind, note: String(note || "").slice(0, 200), bodies: list, constraints, material,
      params: { ...params }
    });
  ...
  // Restoring reuses saved ids so joints reconnect to the objects they named.
  // Recreating in order happens to reproduce the same ids today, but that would
  // break the first time a kind is added, so ids are explicit.
  #claimId(id) {
    const number = Number(String(id).split("-").pop());
    if (Number.isFinite(number) && number > this.#sequence) this.#sequence = number;
    return id;
  }
  ...
  addBox(params = {}) {
    const { x, y, width, height, angle, note, ...material } = params;
    this.#guardCapacity();
    ...
    return this.#register(body, [], this.#nextId("box"), "box", note, options.materialName, params);
  }

  addCircle(params = {}) {
    const { x, y, radius, note, ...material } = params;
    this.#guardCapacity();
    const at = this.#place(x, y);
    const options = this.#material(material);
    const body = Bodies.circle(at.x, at.y, positive(radius, "radius", { max: WORLD_HEIGHT / 2, fallback: 30 }), this.#bodyOptions(options));
    return this.#register(body, [], this.#nextId("circle"), "circle", note, options.materialName, params);
  }
  ...
  addPolygon(params = {}) {
    const { x, y, sides, radius, angle, note, ...material } = params;
    this.#guardCapacity();
    const at = this.#place(x, y);
    const options = this.#material(material);
    ...
    return this.#register(body, [], this.#nextId(kind), kind, note, options.materialName, params);
  }
  ...
  addStar(params = {}) {
    const { x, y, points, radius, innerRadius, note, ...material } = params;
    this.#guardCapacity();
    const at = this.#place(x, y);
    const options = this.#material(material);
    ...
    return this.#register(star, [], this.#nextId("star"), "star", note, options.materialName, params);
  }
  ...
  addGear(params = {}) {
    const { x, y, radius, teeth, speed, note, ...material } = params;
    this.#guardCapacity();
    const at = this.#place(x, y);
    const options = this.#material(material);
    ...
    return this.#register(gear, [pin], id, "gear", note, options.materialName, params);
  }
  ...
  addRamp(params = {}) {
    ...
    return this.#register(body, [], this.#nextId("ramp"), "ramp", note, options.materialName, params);
  }
  ...
  addMotor(params = {}) {
    const { x, y, radius, speed, note, ...material } = params;
    this.#guardCapacity();
    const at = this.#place(x, y);
    const options = this.#material(material);
    ...
    return this.#register(wheel, [pin], id, "motor", note, options.materialName, params);
  }
  ...
  // a rope does not.
  #addLinked(kind, params = {}) {
    const { x, y, links, linkWidth, linkHeight, stiffness, anchored = true, note, ...material } = params;
    ...
    return this.#register(bodies, constraints, this.#nextId(kind), kind, note, options.materialName, params);
  }
  ...
  addRagdoll(params = {}) {
    const { x, y, scale, note, ...material } = params;
    ...
    return this.#register(bodies, constraints, this.#nextId("ragdoll"), "ragdoll", note, options.materialName, params);
  }
  ...
  addCar(params = {}) {
    const { x, y, width, height, wheelSize, note, ...material } = params;
    ...
    return this.#register(
      ...
      this.#nextId("car"), "car", note, options.materialName, params
    );
  }
  ...
  #connect(kind, { a, b, stiffness, length } = {}) {
    ...
    this.#groups.set(id, {
      kind: kind === "pin" ? "pin" : "spring", note: "", bodies: [], constraints: [constraint],
      material: "default", joins: [String(a), String(b)],
      // The solved values, not the requested ones, so a restored joint sits
      // where this one ended up rather than where it was first asked to.
      params: { a: String(a), b: String(b), length: constraint.length, stiffness: constraint.stiffness }
    });
  ...
  //
  // A snapshot records how each object was made *and* where every one of its
  // bodies currently sits. Construction alone would rebuild a ragdoll in its
  // default pose rather than the heap it collapsed into; positions alone could
  // not rebuild its joints at all. Both are needed, and neither is enough.
  snapshot() {
    const objects = [];
    const joints = [];
    for (const [id, group] of this.#groups) {
      if (group.bodies.length === 0) {
        joints.push({ id, kind: group.kind, ...group.params });
        continue;
      }
      objects.push({
        id, kind: group.kind, params: group.params,
        // Stored unrounded. Rounding here and again in frame() double-rounds:
        // 580.1249 saved as 580.125 redraws at 580.13 instead of 580.12, so a
        // reloaded scene sits a hair off the one that was saved.
        bodies: group.bodies.map((body) => ({
          x: body.position.x, y: body.position.y, angle: body.angle,
          vx: body.velocity.x, vy: body.velocity.y, spin: body.angularVelocity
        }))
      });
    }
    return {
  ...
  restore(scene) {
    if (!scene || typeof scene !== "object") throw fail("That is not a saved scene.");
    const version = Number(scene.version) || 0;
    // Refuse rather than half-apply: a scene written by a newer build may name
    // kinds or parameters this one has never heard of, and a partial restore
    // looks like a corrupted save rather than an incompatible one.
    if (version > SCENE_VERSION) {
      throw fail(`That scene was saved by a newer version of Evolv (format ${version}, this build reads ${SCENE_VERSION}).`, 409, "PHYSICS_SCENE_VERSION");
    }

    this.clear();
    this.#engine.gravity.y = finite(scene.gravity, "gravity", { min: -5, max: 5, fallback: 1 });
    this.#wind = finite(scene.wind, "wind", { min: -3, max: 3, fallback: 0 });

    for (const object of Array.isArray(scene.objects) ? scene.objects : []) {
      if (!SHAPES.has(String(object?.kind))) throw fail(`A saved scene names an unknown object: ${object?.kind}`);
      this.#restoringId = String(object.id || "");
      const built = this.apply(`create_${object.kind}`, object.params || {});
      this.#restoringId = null;

      const group = this.#groups.get(built.id);
      const saved = Array.isArray(object.bodies) ? object.bodies : [];
      // Counts only disagree if the engine changed shape under a scene the
      // version check let through; keep what was rebuilt rather than throwing.
      if (group && saved.length === group.bodies.length) {
        group.bodies.forEach((body, index) => {
          const at = saved[index];
          Body.setPosition(body, { x: Number(at.x) || 0, y: Number(at.y) || 0 });
          Body.setAngle(body, Number(at.angle) || 0);
          if (!body.isStatic) {
            Body.setVelocity(body, { x: Number(at.vx) || 0, y: Number(at.vy) || 0 });
            Body.setAngularVelocity(body, Number(at.spin) || 0);
          }
        });
      }
    }
    ...
    for (const joint of Array.isArray(scene.joints) ? scene.joints : []) {
      // A joint naming an object that did not survive is dropped, not fatal.
      if (!this.#groups.has(String(joint?.a)) || !this.#groups.has(String(joint?.b))) continue;
      this.#restoringId = String(joint.id || "");
      this.#connect(joint.kind === "pin" ? "pin" : "spring", joint);
      this.#restoringId = null;
    }

    this.#stepped = Math.round((Number(scene.elapsedSeconds) || 0) * 60);
    this.#initial = null;
    return this.perceive();
  }

  // Reset undoes a run. The scene is captured the first time time is advanced
  // after any change, which is the last moment it is still "as built".
  reset() {
    if (!this.#initial) throw fail("Nothing to reset to — this scene has not been run yet.", 409, "PHYSICS_NO_INITIAL");
    const initial = this.#initial;
    const view = this.restore(initial);
    // restore() clears the mark; put it back so Reset stays repeatable.
  ...
  // Drive a motor from outside. Its speed was fixed when it was added, which is
  // fine for a scene that runs on its own and useless for one being driven by a
  // circuit: the whole point of a coupled run is that this number changes.
  setMotorSpeed(id, radiansPerStep) {
    const key = String(id || "");
    const motor = this.#motors.find((entry) => entry.id === key);
    if (!motor) throw fail(`${key} is not a motor.`, 400, "PHYSICS_NOT_A_MOTOR");
    motor.speed = finite(radiansPerStep, "speed", { min: -1, max: 1, fallback: 0 });
    return { id: key, speed: motor.speed };
  }

  motorIds() { return this.#motors.map((motor) => motor.id); }
  ...
  step(steps = 1) {
    ...
    if (!this.#initial) this.#initial = this.snapshot();
    for (let index = 0; index < count; index += 1) {
  ...
  describe(id) {
    ...
    const rpm = (anchor.angularVelocity * STEPS_PER_SECOND * 60) / (2 * Math.PI);
    ...
    return {
      ...
      speed: round(speed, 3),
      // How fast it is turning, which for a motor is the only thing about it
      // anyone wants to know and was the one number perception did not report:
      // a pinned wheel has a linear speed of zero however hard it is spinning.
      // Matter's angular velocity is radians per step at a fixed 60Hz.
      rpm: round(rpm, 1),
      // Circles carry theirs; anything else has no single radius to report.
      ...(anchor.circleRadius ? { radius: round(anchor.circleRadius, 1) } : {}),
      ...
      //
      // Turning counts as moving. A pinned wheel has a linear speed of zero
      // however fast it spins, so on linear speed alone a motor at 115rpm was
      // reported as resting and left out of the list of things in motion.
      resting: fixed || (speed < 0.05 && Math.abs(rpm) < 1),
  ...
  apply(action, parameters = {}) {
    switch (String(action || "")) {
      ...
      case "restore": return this.restore(parameters.scene);
...
export const PHYSICS_ACTIONS = Object.freeze([
  ...
  "push", "remove", "step", "clear", "reset", "restore"
]);
