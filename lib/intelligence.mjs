import { isFailing } from "./model-health.mjs";
...
export const DEFAULT_INTELLIGENCE_SETTINGS = Object.freeze({
  ...
  // Whether chat may answer a message with a verified multi-step run instead of
  // a single turn. Off makes Evolv a plain chat with tools.
  autoGoals: true,
  // Goal steps are shared out between specialists. Turning this off runs every
  // step as one generalist voice — the control arm of the comparison in the
  // evolution lab, and the answer to "were specialists worth building".
  agentSpecialists: true
});
...
export function normalizeIntelligenceSettings(settings = {}) {
  ...
  return {
    ...
    routingPolicy: "balanced",
    autoGoals: settings.autoGoals !== false,
    agentSpecialists: settings.agentSpecialists !== false
  };
}
...
export async function selectAutoModel({ providerService, database, text, images = [], mode, toolsEnabled }) {
  ...
  // Models that failed their last runs for their own reasons. Auto picking one
  // of these again is the most avoidable bad choice it can make.
  const failing = new Map((database.listModelHealth?.() || [])
    .filter((item) => isFailing(item))
    .map((item) => [`${item.provider}:${item.model}`, item]));
  const candidates = [];
  for (const result of discovered) {
    ...
    for (const model of result.value.models) {
      ...
      // Demoted rather than excluded. A machine where every model has failed
      // still has to answer, and "no model is available" would be a worse
      // reply than a retry of the least-bad one.
      const broken = failing.get(`${providerId}:${model.id}`);
      candidates.push(broken
        ? { providerId, model, ...scored, score: scored.score - 1000, reasons: [...scored.reasons, `recently failed: ${broken.reason}`] }
        : { providerId, model, ...scored });
    }
  }
