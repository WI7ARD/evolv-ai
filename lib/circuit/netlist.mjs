...
import { sensorDriver, sensorOhms, SENSOR_DRIVEN_PINS } from "./sensors.mjs";
...
export function stampComponents(components, index, system, circuit, moment = null) {
  for (const component of components) {
    ...
    switch (kind) {
      ...
      // A thermistor and an LDR are resistors whose value is set by the world
      // rather than by a colour code. Nothing else about them is special, which
      // is exactly what makes them usable in a divider without a second thought.
      case "thermistor":
      case "ldr": {
        const ohms = sensorOhms(component, conditionsAt(moment?.conditions, moment?.time ?? 0));
        system.stampConductance(nodeOf(index, component, "a"), nodeOf(index, component, "b"), 1 / Math.max(1e-6, ohms));
        break;
      }
      case "switch": {
      ...
      case "capacitor": {
        ...
        // No timestep means no integration: this is an operating point, not a
        // step. The test is for the history rather than for the moment itself
        // because a moment now also carries the surroundings, which an
        // operating point does have — a thermistor is at room temperature
        // whether or not anything is changing.
        if (!moment?.history) {
      ...
      case "mcupin": {
        ...
        // What the part costs to be switched on, before it drives anything.
        //
        // The same omission as the microcontroller's, and it matters for the
        // same reason: a hall sensor watching for a magnet draws five
        // milliamps whether or not a magnet ever comes, which is a third of
        // what the microcontroller costs and is the first thing anyone
        // switches off between readings when a board has to last on a cell.
        // Reported as nanoamps, it would look free.
        const idle = Math.max(0, Number(values.activeAmps) || 0);
        if (idle > 0 && component.pins.vcc && component.pins.gnd) {
          system.stampConductance(nodeOf(index, component, "vcc"), nodeOf(index, component, "gnd"), idle / NOMINAL_LOGIC_VOLTS);
        }
        const memory = moment?.chips?.get(component.id) ?? null;
        // A sensor is asked what the world is doing to it; a chip is asked what
        // its own inputs are doing. Both answer in the same shape, so
        // everything below this line is common to the two.
        const driver = SENSOR_DRIVEN_PINS[kind]
          ? sensorDriver(component, read, moment?.conditions, moment?.time ?? 0)
          : chipDriver(component, read, memory);
...
function currentsFor(components, index, solution, moment = null) {
  ...
  for (const component of components) {
    ...
    // counting it twice.
    // A driven part with its own supply draw reports the total off the rail,
    // like the microcontroller, rather than what one leg is doing.
    if (Number(values.activeAmps) > 0 && component.pins.vcc && component.pins.gnd && kind !== "mcu") {
      const supply = voltageAt(solution, nodeOf(index, component, "vcc")) - voltageAt(solution, nodeOf(index, component, "gnd"));
      let total = Math.max(0, supply) * (Number(values.activeAmps) / NOMINAL_LOGIC_VOLTS);
      for (const pin of DRIVEN_PINS[kind] || []) {
        const allocation = index.drivers.get(`${component.id}:${pin}`);
        if (allocation) total += Math.max(0, -solution[allocation.branch]);
      }
    ...
    if (moment?.history && (kind === "capacitor" || kind === "inductor")) {
      const past = moment.history.get(component.id) || { voltage: 0, current: 0 };
    ...
    let ohms = null;
    if (kind === "thermistor" || kind === "ldr") {
      ohms = Math.max(1e-6, sensorOhms(component, conditionsAt(moment?.conditions, moment?.time ?? 0)));
    }
    else if (kind === "resistor") ohms = Math.max(1e-6, Number(values.ohms) || 0);
...
export function findings(components, index, solution, currents) {
  ...
  for (const [net, connections] of nets) {
    ...
    if (connections.length < 2) {
      // An unused output is not a fault. A three-axis accelerometer on a board
      // that only cares about two has a spare pin, and nobody routes it — the
      // pin still sits at a defined voltage with nothing drawing from it, which
      // is a working circuit and not a mistake.
      //
      // A microcontroller's legs are deliberately not covered by this. They are
      // bidirectional, so a wire from one to nowhere is far more likely to be an
      // input someone forgot to connect — which really is undefined, and really
      // is the bug worth catching.
      const [only] = connections;
      const part = components.find((component) => component.id === only?.component);
      if (part && part.kind !== "mcu" && DRIVEN_PINS[part.kind]?.includes(only.pin)) continue;
      found.push({
        severity: "error", code: "FLOATING_NET", net,
        message: `Net ${net} has only one thing connected to it (${only?.component} ${only?.pin}). Current has nowhere to go, so nothing will happen here.`
      });
    }
  }
...
export function solveNetlist(components, { conditions = null } = {}) {
  ...
  let iterations = 0;
  try {
    // A DC operating point has no instant, but it does have surroundings: a
    // thermistor is at whatever temperature the room is whether or not anything
    // is changing. Time is zero, so a ramp is read at its start.
    const moment = conditions ? { conditions, time: 0 } : null;
    const result = solveNonlinear(index.size, (system, current, circuit) =>
      stampComponents(components, index, system, circuit, moment));
  ...
  const currents = currentsFor(components, index, solution, conditions ? { conditions, time: 0 } : null);
  const nodes = {};
