...
// The names every sketch is written with.
//
// pinMode(0, OUTPUT) and digitalWrite(led, HIGH) are how this is spelled
// everywhere, and neither worked: the only accepted spelling was a bare 1, and
// OUTPUT came back as "OUTPUT has not been given a value (line 2)". Anyone who
// has written a sketch would type the words first, and be told the language
// does not have them.
//
// Fixed values, not seeded globals. On a real board these are #defines and
// cannot be assigned to; as globals, `HIGH = 5` would be accepted and every
// digitalWrite after it would quietly drive the wrong level — exactly the kind
// of silent wrong answer the rest of this interpreter refuses to give.
//
// The numbers have to agree with PIN_MODES in mcu.mjs, which cannot be imported
// here because mcu.mjs imports this file. A test pins them together instead.
...
export class Firmware {
  ...
  *#run() {
    ...
    // Everything written outside a function, run once before anything else.
    //
    // This was missing, and the effect was worse than a variable being
    // unavailable: reading a global was an error, but *assigning* to an unknown
    // name quietly made a local instead, so `count = count + 1` in loop() threw
    // while `count = 1` appeared to work and then reset on the next iteration.
    // A counter is most of what firmware for a sensor board does, so almost
    // nothing real could be written until this ran.
    for (const statement of this.program.body) {
      if (statement.type === "FunctionDeclaration") continue;
      yield* this.#execute(statement, this.globals);
    }
  ...
  *#callFunction(declaration, args, _parent) {
  ...
  #lookup(scope, name) {
    ...
    // Last, so a program that declared its own `var OUTPUT` still sees its own.
    // Shadowing on purpose is a choice; the constants are only there for the
    // programs that never mention them.
    if (name in CONSTANTS) return CONSTANTS[name];
    return undefined;
  }

  #assign(scope, name, value, line = 0) {
    // Assigning to HIGH is not a declaration and never a good idea: every
    // digitalWrite afterwards would drive whatever the new value was.
    if (!scope.has(name) && !this.globals.has(name) && name in CONSTANTS) {
      throw new FirmwareError(
        `${name} is a fixed value and cannot be assigned to`,
        line, "FIRMWARE_ASSIGN_TO_CONSTANT"
      );
    }
    if (scope.has(name)) scope.set(name, value);
    else if (this.globals.has(name)) this.globals.set(name, value);
    // Assigning to a name nothing declared is a mistake, not a declaration.
    //
    // Making it a local was the quiet failure: the value survived to the end of
    // the function and vanished, so a counter incremented perfectly and read
    // zero every time round. Reading an undeclared name was already an error;
    // writing one is the same mistake seen from the other side, and usually a
    // missing var or a typo in a name that does exist.
    else {
      throw new FirmwareError(
        `${name} has not been declared — write "var ${name} = ..." before assigning to it`,
        line, "FIRMWARE_UNDECLARED_ASSIGN"
      );
    }
  }
  ...
  *#evaluate(node, scope) {
    switch (node.type) {
      ...
      case "Assignment": {
        const value = yield* this.#evaluate(node.value, scope);
        this.#assign(scope, node.name, value, node.line);
        return value;
      }
      case "PostStep": {
        const before = this.#lookup(scope, node.name) ?? 0;
        this.#assign(scope, node.name, node.operator === "+" ? before + 1 : before - 1, node.line);
        return before;
      }
      case "Unary": {
