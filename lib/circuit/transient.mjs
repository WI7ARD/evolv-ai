...
import { createDeviceState, advanceDevices, radiansToRpm, ledBrightness } from "./devices.mjs";
import { createMcus } from "./mcu.mjs";
...
export function stepTransient(components, index, history, { dt, time, method, chips = null, devices = null, mcus = null, conditions = null }) {
  ...
  const moment = { dt, time, method, history, chips, devices, mcus, conditions };
  ...
  // Firmware runs before the solve, on what its pins read at the end of the
  // last step, and the circuit is then solved with whatever it decided to
  // drive. That is the real order of events on a board: the chip acts, then the
  // electricity settles around it.
  if (mcus) for (const chip of mcus.values()) chip.advance(dt);
  let solution = solve();
  if (chips) {
  ...
  // And the chip reads its own legs back, so digitalRead answers about the
  // world rather than about what the firmware last wrote there.
  if (mcus) {
    for (const [id, chip] of mcus) {
      const component = components.find((item) => item.id === id);
      if (component) chip.observe((pin) => readFrom(solution)(component, pin));
    }
  }
...
// What each microcontroller has to say about its own firmware, as findings, so
// it reaches every reader of a run rather than only the one panel that asks a
// chip directly.
function firmwareFindings(mcus) {
  if (!mcus) return [];
  const found = [];
  for (const [id, chip] of mcus) {
    const error = chip.view?.().error || chip.error;
    if (error) {
      found.push({
        severity: "error", code: error.code || "FIRMWARE_ERROR", component: id,
        // The firmware error already names its own line; adding it again gave
        // "(line 2) (line 2)".
...
export function runTransient(components, { seconds, dt = 0, method = "euler", probes = [], history = null, chipState = null, deviceState = null, mcus = null, conditions = null, startTime = 0 }) {
  ...
  if (structural.some((item) => item.severity === "error")) {
    return {
      ...
      mcus: mcus || createMcus(components)
    };
  }
  ...
  const chipsRunning = mcus || createMcus(components);
  let time = startTime;
  ...
  // The rails are up before the chip's first instruction.
  //
  // Firmware runs at the top of a step against what its pins read at the end of
  // the step before, and at the very first step there is no step before — so
  // the first pass through loop() read zero volts everywhere, and an analogRead
  // on a live divider answered 0 instead of 511. That is not a lesson about
  // anything; a real board's supply settles long before the first instruction
  // retires. So a fresh chip gets one bias solve, with every pin idle because
  // setup() has not run yet, handed to it before it executes at all.
  if ([...chipsRunning.values()].some((chip) => !chip.observed)) {
    const moment = { dt: step, time, method: "euler", history: state, chips: null, devices: null, mcus: chipsRunning, conditions };
    const bias = solveNonlinear(index.size, (system, current, circuit) =>
      stampComponents(components, index, system, circuit, moment)).solution;
    for (const [id, chip] of chipsRunning) {
      const component = components.find((item) => item.id === id);
      if (component) chip.observe((pin) => {
        const net = component.pins[pin];
        const node = net && index.nodes.has(net) ? index.nodes.get(net) : -1;
        return node >= 0 ? bias[node] : 0;
      });
    }
  }
  ...
  for (let stepIndex = 0; stepIndex < requested; stepIndex += 1) {
    ...
    time += step;
    const result = stepTransient(components, index, state, { dt: step, time, method: stepMethod, chips, devices, mcus: chipsRunning, conditions });
      ...
      const value = probe.kind === "net"
        ...
        : probe.kind === "speed"
          ...
          : probe.kind === "angle"
            ...
            // How bright, on the 0-to-1 scale the schematic draws its glow
            // from, rather than the current behind it. "Is the LED lit" is a
            // question about the light, and an expectation that had to be
            // written about milliamps instead would be asking the person to do
            // the diode's arithmetic themselves.
            : probe.kind === "lit"
              ? ledBrightness(result.currents.get(probe.target) ?? 0,
                  PARTS[componentsById.get(probe.target)?.kind]?.ratings?.amps || 0.02)
  ...
  return {
    ...
    // A microcontroller that never started is the loudest fact about a run, and
    // it used to be silent: a check on a board whose firmware died on line 2
    // reported "never switched, it sat between 0% and 0%" — perfectly true, and
    // pointing at the LED instead of at the typo. The chip already knew; it was
    // only ever written where the schematic panel looks.
    findings: [...findings(components, index, last.solution, last.currents), ...firmwareFindings(chipsRunning)],
    traces: recorded,
    history: state,
    chipState: chips,
    deviceState: devices,
