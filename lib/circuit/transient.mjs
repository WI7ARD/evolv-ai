...
export function stepTransient(components, index, history, { dt, time, method }) {
  const moment = { dt, time, method, history };
  const { solution } = solveNonlinear(index.size, (system, current, circuit) =>
...
export function runTransient(components, { seconds, dt = 0, method = "euler", probes = [], history = null, startTime = 0 }) {
  ...
  if (structural.some((item) => item.severity === "error")) {
    return { ran: false, findings: structural, traces: [], time: startTime, steps: 0, history: history || createHistory(components) };
  }
  ...
  for (let stepIndex = 0; stepIndex < requested; stepIndex += 1) {
    ...
    time += step;
    const result = stepTransient(components, index, state, { dt: step, time, method: stepMethod });
