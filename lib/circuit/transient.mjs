...
export function stepTransient(components, index, history, { dt, time, method, chips = null, devices = null, mcus = null }) {
  ...
  const moment = { dt, time, method, history, chips, devices, mcus };
...
export function runTransient(components, { seconds, dt = 0, method = "euler", probes = [], history = null, chipState = null, deviceState = null, mcus = null, startTime = 0 }) {
  ...
  if ([...chipsRunning.values()].some((chip) => !chip.observed)) {
    const moment = { dt: step, time, method: "euler", history: state, chips: null, devices: null, mcus: chipsRunning };
  ...
  for (let stepIndex = 0; stepIndex < requested; stepIndex += 1) {
    ...
    time += step;
    const result = stepTransient(components, index, state, { dt: step, time, method: stepMethod, chips, devices, mcus: chipsRunning });
