...
export function stepTransient(components, index, history, { dt, time, method, chips = null }) {
  ...
  const moment = { dt, time, method, history, chips };
...
export function runTransient(components, { seconds, dt = 0, method = "euler", probes = [], history = null, chipState = null, startTime = 0 }) {
  ...
  for (let stepIndex = 0; stepIndex < requested; stepIndex += 1) {
    ...
    time += step;
    const result = stepTransient(components, index, state, { dt: step, time, method: stepMethod, chips });
