...
export const PARTS = Object.freeze({
  ...
  // The parts that decide something. Their behaviour lives in
  // lib/circuit/chips.mjs; what is here is the shape a schematic and a parts
  // list need — which pins exist, in which order, and what to call it.
  opamp: {
    label: "Op-amp", symbol: "opamp", pins: ["inPlus", "inMinus", "out", "vPos", "vNeg"], unit: "",
    defaults: { part: "LM358" },
    ratings: { amps: 0.04 },
    describe: (values) => describeChip("opamp", values)
  },
  regulator: {
    label: "Voltage regulator", symbol: "regulator", pins: ["input", "output", "ground"], unit: "V",
    defaults: { part: "LM7805", volts: 5 },
    ratings: { amps: 1 },
    describe: (values) => describeChip("regulator", values)
  },
  gate: {
    label: "Logic gate", symbol: "gate", pins: ["a", "b", "out", "vcc", "gnd"], unit: "",
    defaults: { function: "nand" },
    ratings: { amps: 0.025 },
    describe: (values) => describeChip("gate", values)
  },
  flipflop: {
    label: "D flip-flop", symbol: "flipflop", pins: ["d", "clk", "q", "qn", "vcc", "gnd"], unit: "",
    defaults: {},
    ratings: { amps: 0.025 },
    describe: (values) => describeChip("flipflop", values)
  },
  timer555: {
    label: "555 timer", symbol: "timer555",
    pins: ["trigger", "threshold", "discharge", "out", "reset", "vcc", "gnd"], unit: "",
    defaults: {},
    ratings: { amps: 0.2 },
    describe: (values) => describeChip("timer555", values)
  },
  mcupin: {
    label: "MCU pin", symbol: "mcupin", pins: ["pin", "vcc", "gnd"], unit: "",
    defaults: { mode: "low", duty: 0.5 },
    ratings: { amps: 0.04 },
    describe: (values) => describeChip("mcupin", values)
  },
  // Outputs: the parts whose behaviour is physical rather than electrical. What
  // they do lives in lib/circuit/devices.mjs.
  motor: {
    label: "DC motor", symbol: "motor", pins: ["positive", "negative"], unit: "",
    defaults: { volts: 6, resistance: 3, inductance: 1.5e-3, ke: 0.008, inertia: 5e-7, friction: 1e-7, loadTorque: 0 },
    // Stall current is V/R and it is large. The rating is what the winding
    // tolerates continuously, which is nothing like what it draws at rest.
    ratings: { amps: 1 },
    describe: (values) => describeDevice("motor", values)
  },
  servo: {
    label: "Servo", symbol: "servo", pins: ["signal", "vcc", "gnd"], unit: "",
    defaults: {},
    ratings: { amps: 1 },
    describe: () => describeDevice("servo")
  },
  buzzer: {
    label: "Buzzer", symbol: "buzzer", pins: ["a", "b"], unit: "",
    defaults: { resistance: 32 },
    ratings: { amps: 0.1 },
    describe: () => describeDevice("buzzer")
  },
  rgbled: {
    label: "RGB LED", symbol: "rgbled", pins: ["red", "green", "blue", "common"], unit: "",
    defaults: { common: "cathode" },
    ratings: { amps: 0.02 },
    describe: () => describeDevice("rgbled")
  },
  sevenseg: {
    label: "7-segment display", symbol: "sevenseg",
    pins: ["a", "b", "c", "d", "e", "f", "g", "dp", "common"], unit: "",
    defaults: { common: "cathode" },
    ratings: { amps: 0.02 },
    describe: (values) => describeDevice("sevenseg", values)
  },
  // The microcontroller. Its pins are ordinary nodes; what makes it different is
  // that firmware decides what they do. See lib/circuit/mcu.mjs.
  mcu: {
    label: "Microcontroller", symbol: "mcu",
    pins: ["d0", "d1", "d2", "d3", "d4", "d5", "d6", "d7", "a0", "a1", "a2", "a3", "vcc", "gnd"],
    unit: "",
    // activeAmps is what the chip draws to be switched on and running, before
    // it drives a single pin. Twelve milliamps is an 8-bit part at 16MHz on 5V;
    // a radio-carrying board is ten times that, which is why it is a value
    // rather than a constant. It matters most on battery: on a board that sleeps
    // between readings this figure, not the sensors, sets how long it lasts.
    defaults: { clockHz: 16_000_000, activeAmps: 0.012 },
    ratings: { amps: 0.2 },
    describe: (values) => `Microcontroller at ${Math.round((Number(values.clockHz) || 16e6) / 1e6)}MHz`
  },
  // The parts that respond to something outside the circuit. What they do lives
  // in lib/circuit/sensors.mjs; what is here is the shape a schematic and a
  // parts list need.
  //
  // Two of them are resistances that change and need no supply; two are chips
  // that drive an output and do. That difference is the first thing to get right
  // when wiring either, so the pin lists say it plainly.
  thermistor: {
    label: "Thermistor", symbol: "thermistor", pins: ["a", "b"], unit: "Ω",
    defaults: { ohms: 10_000, beta: 3950, coefficient: "ntc", watts: 0.1 },
    ratings: { watts: 0.1 },
    describe: (values) => `${formatOhms(values.ohms)} ${String(values.coefficient || "ntc").toUpperCase()} thermistor`
  },
  ldr: {
    label: "Light sensor", symbol: "ldr", pins: ["a", "b"], unit: "Ω",
    defaults: { ohms: 10_000, gamma: 0.8, darkOhms: 1_000_000, watts: 0.1 },
    ratings: { watts: 0.1 },
    describe: (values) => `${formatOhms(values.ohms)} light sensor`
  },
  hall: {
    label: "Wheel sensor", symbol: "hall", pins: ["out", "vcc", "gnd"], unit: "",
    // A magnet on the wheel and a sensor on the fork: the commonest speed
    // sensor there is. The circumference is the number a bike computer asks
    // for, and 2096mm is what they ship set to.
    // Five milliamps to sit there watching for a magnet. That is not a detail on
    // a board that runs off a cell: it is a third of what the microcontroller
    // costs, all the time, and it is the first thing to switch off between
    // readings if the battery has to last.
    defaults: { circumference: 2.096, magnets: 1, duty: 0.1, style: "push-pull", activeAmps: 0.005 },
    ratings: { amps: 0.025, volts: 24 },
    describe: (values) => `hall sensor, ${Math.round((Number(values.circumference) || 2.096) * 1000)}mm wheel${
      (Number(values.magnets) || 1) > 1 ? `, ${values.magnets} magnets` : ""}`
  },
  accelerometer: {
    label: "Accelerometer", symbol: "accelerometer", pins: ["x", "y", "z", "vcc", "gnd"], unit: "",
    defaults: { range: 3, sensitivityFraction: 0.1, activeAmps: 0.00035 },
    ratings: { volts: 6 },
    describe: (values) => `±${Number(values.range) || 3}g accelerometer`
  },
  potentiometer: {
...
import { describeChip } from "./chips.mjs";
import { describeDevice } from "./devices.mjs";
