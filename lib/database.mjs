...
export function createDatabase({ dataDir, legacyStateFile, defaultPrompt, baselineUpgrade = null, dbPath = path.join(dataDir, "evolv.db") }) {
  ...
  const api = {
    ...
    updateMessage(id, patch = {}) {
      ...
      db.prepare(`
        UPDATE messages SET content = ?, thinking = ?, model = ?, status = ?, metadata_json = ?, updated_at = ? WHERE id = ?
      `).run(
    ...
    createEvaluationRun(item) {
      const id = item.id || crypto.randomUUID();
      const modelId = String(item.modelId || item.model || "").trim();
      if (!modelId) throw Object.assign(new Error("Evaluation model is required."), { status: 400, code: "EVALUATION_MODEL_REQUIRED" });
      db.prepare(`INSERT INTO evaluation_runs(id, proposal_id, provider_id, model_id, status, summary_json, created_at)
        VALUES (?, ?, ?, ?, ?, '{}', ?)`)
        .run(id, item.proposalId || null, item.providerId, modelId, item.status || "running", item.createdAt || now());
      return id;
    },
