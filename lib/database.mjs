...
export function createDatabase({ dataDir, legacyStateFile, defaultPrompt, baselineUpgrade = null, dbPath = path.join(dataDir, "evolv.db") }) {
  ...
  const api = {
    ...
    // Boards. A board is a design plus what has happened to it. The design
    // stays opaque to SQL, exactly as a physics scene does above, so the
    // simulator keeps no database handle and stays memory-only — which is what
    // keeps its tools in the automatic risk tier. The history sits beside it. Nothing here knows what a stage means — that
    // is lib/boards.mjs — so a new stage never needs a migration.
    saveBoard({ id = "", name, intent = "", snapshot, stages = {}, partCount = 0, projectId = null }) {
      const title = String(name || "").trim().slice(0, 80);
      if (!title) throw Object.assign(new Error("A board needs a name."), { status: 400, code: "BOARD_NAME_REQUIRED" });
      const stamp = new Date().toISOString();
      return db.transaction(() => {
        // By id when one is given, by name otherwise. Saving over a board you
        // have open should update it even after a rename, and saving a design
        // under a name already in use should not silently create a second board
        // that the unique index would refuse anyway.
        const existing = id
          ? db.prepare("SELECT id, created_at FROM boards WHERE id = ?").get(String(id))
          : db.prepare("SELECT id, created_at FROM boards WHERE name = ?").get(title);
        const boardId = existing?.id || String(id) || crypto.randomUUID();
        db.prepare(`
          INSERT INTO boards(id, project_id, name, intent, snapshot_json, stages_json, part_count, spend_micros, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?)
          ON CONFLICT(id) DO UPDATE SET
            ...
            updated_at = excluded.updated_at
        `).run(
          boardId, projectId ? String(projectId) : null, title, String(intent || "").slice(0, 600),
          JSON.stringify(snapshot), JSON.stringify(stages), Number(partCount) || 0,
          existing?.created_at || stamp, stamp
        );
        return { id: boardId, name: title, updatedAt: stamp };
      })();
    },
    listBoards({ limit = 50, projectId = null } = {}) {
      const size = Math.max(1, Math.min(200, Number(limit) || 50));
      const columns = `id, project_id AS projectId, name, intent, part_count AS partCount,
        spend_micros AS spendMicros, stages_json AS stagesJson, created_at AS createdAt, updated_at AS updatedAt`;
      return projectId
        ? db.prepare(`SELECT ${columns} FROM boards WHERE project_id = ? ORDER BY updated_at DESC, rowid DESC LIMIT ?`)
            .all(String(projectId), size)
        : db.prepare(`SELECT ${columns} FROM boards ORDER BY updated_at DESC, rowid DESC LIMIT ?`).all(size);
    },
    getBoard(id) {
      const row = db.prepare(`
        SELECT id, project_id AS projectId, name, intent, snapshot_json AS snapshotJson,
               stages_json AS stagesJson, part_count AS partCount, spend_micros AS spendMicros,
      ...
      try {
        ...
        // A board whose history will not parse still has a usable design, and
        // refusing to open it would lose the design over the history. An empty
        // history reads as "nothing has happened yet", which is wrong but
        // recoverable; a board that will not open is neither.
        stages = JSON.parse(row.stagesJson || "{}");
      } catch {
        if (snapshot === undefined) {
          throw Object.assign(new Error("That board's design is unreadable."), { status: 422, code: "BOARD_CORRUPT" });
        }
        stages = {};
      }
      const { snapshotJson, stagesJson, ...board } = row;
      return {
        ...board, snapshot, stages,
        measurements: db.prepare(`
          SELECT id, subject, measure, value, note, measured_at AS measuredAt
          FROM board_measurements WHERE board_id = ? ORDER BY measured_at DESC, rowid DESC
        `).all(row.id)
      };
    },
    deleteBoard(id) {
      return db.prepare("DELETE FROM boards WHERE id = ?").run(String(id || "")).changes > 0;
    },
    // One stage at a time, merged into what is already there. A whole-object
    // write would make an export race a check and lose one of them.
    recordBoardStage(id, stage, record) {
      const boardId = String(id || "");
      return db.transaction(() => {
        const row = db.prepare("SELECT stages_json AS stagesJson FROM boards WHERE id = ?").get(boardId);
        if (!row) return null;
        let stages = {};
        try { stages = JSON.parse(row.stagesJson || "{}"); } catch { stages = {}; }
        stages[String(stage)] = record;
        const stamp = new Date().toISOString();
        db.prepare("UPDATE boards SET stages_json = ?, updated_at = ? WHERE id = ?").run(JSON.stringify(stages), stamp, boardId);
        return { stage: String(stage), at: stamp };
      })();
    },
    addBoardMeasurement(boardId, measurement, { keep = 200 } = {}) {
      const target = String(boardId || "");
      return db.transaction(() => {
        if (!db.prepare("SELECT 1 FROM boards WHERE id = ?").get(target)) return null;
        const entry = {
          id: crypto.randomUUID(), boardId: target,
          subject: String(measurement.subject), measure: String(measurement.measure),
          value: Number(measurement.value), note: String(measurement.note || ""),
          measuredAt: measurement.measuredAt || new Date().toISOString()
        };
        db.prepare(`
          INSERT INTO board_measurements(id, board_id, subject, measure, value, note, measured_at)
          VALUES (@id, @boardId, @subject, @measure, @value, @note, @measuredAt)
        `).run(entry);
        db.prepare(`
          DELETE FROM board_measurements WHERE board_id = @boardId AND id NOT IN (
            SELECT id FROM board_measurements WHERE board_id = @boardId
            ORDER BY measured_at DESC, rowid DESC LIMIT @keep
          )
        `).run({ boardId: target, keep: Math.max(1, keep) });
        return entry;
      })();
    },
    deleteBoardMeasurement(id) {
      return db.prepare("DELETE FROM board_measurements WHERE id = ?").run(String(id || "")).changes > 0;
    },
    // Cost is added rather than set, because a board is designed over many turns
    // and each one only knows what it itself used.
    addBoardSpend(id, micros) {
      const amount = Math.max(0, Math.round(Number(micros) || 0));
      if (!amount) return false;
      return db.prepare("UPDATE boards SET spend_micros = spend_micros + ? WHERE id = ?").run(amount, String(id || "")).changes > 0;
    },
