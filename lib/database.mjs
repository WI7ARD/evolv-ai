  ...
  for (const migration of [...migrations].sort((left, right) => left.version - right.version)) {
    ...
    if (applied.has(migration.version)) {
      ...
      if (!previous) record.run(migration.version, migration.name, checksum, now());
        ...
        throw Object.assign(
          new Error(`Migration ${migration.version} (${migration.name}) was altered after it was applied.`),
...
export function createDatabase({ dataDir, legacyStateFile, defaultPrompt, baselineUpgrade = null, dbPath = path.join(dataDir, "evolv.db") }) {
  ...
  const api = {
    ...
    // Paired phone companions. Only the hash of a device token is stored, so a
    // copied database yields nothing that can be replayed against the server.
    addCompanionDevice({ id, name, tokenHash }) {
      db.prepare(`INSERT INTO companion_devices(id, name, token_hash, created_at) VALUES(?,?,?,?)`)
        .run(String(id), String(name).slice(0, 80), String(tokenHash), now());
      return api.getCompanionDevice(id);
    },
    getCompanionDevice(id) {
      return db.prepare(`SELECT id, name, created_at AS createdAt, last_seen_at AS lastSeenAt,
        revoked_at AS revokedAt FROM companion_devices WHERE id = ?`).get(String(id || "")) || null;
    },
    findCompanionDeviceByTokenHash(tokenHash) {
      return db.prepare(`SELECT id, name, created_at AS createdAt, last_seen_at AS lastSeenAt,
        revoked_at AS revokedAt FROM companion_devices WHERE token_hash = ? AND revoked_at IS NULL`)
        .get(String(tokenHash || "")) || null;
    },
    listCompanionDevices() {
      return db.prepare(`SELECT id, name, created_at AS createdAt, last_seen_at AS lastSeenAt,
        revoked_at AS revokedAt FROM companion_devices ORDER BY created_at DESC`).all();
    },
    touchCompanionDevice(id) {
      db.prepare("UPDATE companion_devices SET last_seen_at = ? WHERE id = ?").run(now(), String(id || ""));
    },
    // Revoked rather than deleted: the row is the record that a device once had
    // access, which is the only way to answer "what was paired last March?".
    revokeCompanionDevice(id) {
      return db.prepare("UPDATE companion_devices SET revoked_at = ? WHERE id = ? AND revoked_at IS NULL")
