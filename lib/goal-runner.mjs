import { parsePlannerJson, plannerSystemPrompt, validateGoalPlan, normalizeGoalBudgets } from "./goal-contracts.mjs";
...
export class GoalRunnerService {
  ...
  availableTools() {
    const approvalTools = new Set(["propose_web_research", "propose_workspace_edit", "propose_workspace_create", "propose_engineering_check", "propose_obsidian_create"]);
    return this.toolRegistry.list().filter((tool) => tool.enabled && (tool.risk === "read" || approvalTools.has(tool.name))).map((tool) => tool.name);
  }
  ...
  async proposePlan({ objective, successCriteria, providerId, model, budgets }) {
    ...
    const plan = validateGoalPlan(parsePlannerJson(result.content), { availableTools: tools, budgets: normalizedBudgets });
  ...
  async create(body) {
    ...
    const proposed = body.plan
      ? { plan: validateGoalPlan(body.plan, { availableTools: this.availableTools(), budgets: normalizeGoalBudgets(body.budgets) }), route: { providerId, model, fallback: false }, budgets: normalizeGoalBudgets(body.budgets) }
      : await this.proposePlan({ objective, successCriteria, providerId, model, budgets: body.budgets });
    ...
    const run = this.agentRuntime.createGoalRun({
      conversationId: conversation.id, projectId: project.id, packId: String(body.packId || ""), objective, successCriteria,
      providerId, modelId: model, budgets: proposed.budgets, plan: proposed.plan, availableTools: this.availableTools(),
      request: { provider: providerId, model, projectId: project.id, packId: String(body.packId || ""), successCriteria, fallback: proposed.route.fallback }
    });
  ...
  revise(runId, plan) {
    return this.agentRuntime.reviseGoalPlan(runId, plan, { availableTools: this.availableTools() });
  }
  ...
  async #modelStep(run, step, signal) {
    ...
    let route = { providerId: run.providerId, model: run.modelId, fallback: false };
    let response;
    try { response = await this.#complete(route.providerId, route.model, messages, { temperature: verification ? 0 : 0.2, maxTokens: 4096, signal }); }
    catch {
      route = await this.#fallbackRoute(route.providerId, route.model, run.id, verification ? "verification" : "execution");
