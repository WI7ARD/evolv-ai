import { agentSystemPrompt, resolveAgent } from "./agents.mjs";
...
export class GoalRunnerService {
  ...
  async #modelStep(run, step, signal) {
    ...
    const agent = resolveAgent(this.#plannedStep(run, step)?.agent, { step: { type: step.kind } });
    ...
    try { response = await this.#complete(route.providerId, route.model, messages, { temperature: verification ? 0 : agent.temperature, maxTokens: 4096, signal }); }
    catch {
