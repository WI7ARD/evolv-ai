...
import { collectAiEvent, createAiAccumulator } from "./ai-events.mjs";
...
export class GoalRunnerService {
  ...
  // control arm of the comparison: the plan is written against one voice, the
  // steps run as one voice, and a pack cannot slip a second one in.
  ...
  async #complete(providerId, model, messages, { temperature = 0.2, maxTokens = 4096, signal = AbortSignal.timeout(180_000) } = {}) {
    ...
    }, signal, (event) => collectAiEvent(result, event));
    return {
      content: result.content.trim(),
      thinking: result.reasoning.trim(),
      usage: result.usage,
