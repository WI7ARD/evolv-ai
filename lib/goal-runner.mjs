...
export class GoalRunnerService {
  ...
  // control arm of the comparison: the plan is written against one voice and the
  // steps run as one voice.
  //
  // It used to be able to grow by a pack's own agents. Packs are gone, so the
  // roster is the built-in one — which is what it always was on every machine
  // that had not installed anything.
  #roster(packId = "", specialists = true) {
    if (!specialists) return [GENERALIST_AGENT];
    return listAgents();
  }
  ...
  async proposePlan({ objective, successCriteria, providerId, model, budgets, packId = "", specialists = this.#specialists() }) {
    const tools = this.availableTools();
    const normalizedBudgets = normalizeGoalBudgets(budgets);
    const roster = this.#roster(packId, specialists);
    const messages = [
  ...
  async create(body) {
    ...
    const proposed = body.plan
      ? {
        plan: validateGoalPlan(body.plan, {
          availableTools: this.availableTools(), budgets: normalizeGoalBudgets(body.budgets), roster: this.#roster(packId, specialists)
        }),
        route: { providerId, model, fallback: false },
        budgets: normalizeGoalBudgets(body.budgets)
      }
      : await this.proposePlan({ objective, successCriteria, providerId, model, budgets: body.budgets, packId, specialists });
    ...
    const run = this.agentRuntime.createGoalRun({
      ...
      request: { provider: providerId, model, projectId: project.id, packId, successCriteria, fallback: proposed.route.fallback, specialists }
    });
  ...
  revise(runId, plan) {
    ...
    return this.agentRuntime.reviseGoalPlan(runId, plan, {
      availableTools: this.availableTools(), roster: this.#roster(run?.goal?.packId || "", this.#specialists(run))
    });
  }
  ...
  async #modelStep(run, step, signal) {
    ...
    const agent = resolveAgent(this.#plannedStep(run, step)?.agent, {
      step: { type: step.kind }, roster: this.#roster(run.goal?.packId, specialists)
    });
      ...
      try {
        ...
        if (["analyze", "verification", "report"].includes(active.kind)) {
          ...
          const stepAgent = resolveAgent(this.#plannedStep(current, active)?.agent, {
            step: { type: active.kind }, roster: this.#roster(current.goal?.packId, this.#specialists(current))
          });
