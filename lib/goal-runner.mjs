import { agentMayRunStep, agentModelOverride, agentSystemPrompt, listAgents, resolveAgent, GENERALIST_AGENT } from "./agents.mjs";
import { parsePlannerJson, plannerSystemPrompt, stepToolUse, validateGoalPlan, normalizeGoalBudgets } from "./goal-contracts.mjs";
import { CRITERIA_SYSTEM_PROMPT, planEarnsARun, planOnlyReads, shouldRunAsGoal } from "./goal-escalation.mjs";
...
export class GoalRunnerService {
  constructor({ database, agentRuntime, toolRegistry, providerService, vaultService, projectService }) {
    Object.assign(this, { database, agentRuntime, toolRegistry, providerService, vaultService, projectService });
  }

  // The specialists this goal may be shared out between.
  //
  // With specialists turned off the roster is a single generalist, which is the
  // control arm of the comparison: the plan is written against one voice and
  // the steps run as one voice.
  #roster(specialists = true) {
    return specialists ? listAgents() : [GENERALIST_AGENT];
  }

  // Which arm a run belongs to. Read off the run once it exists, because a run
  // that started with specialists has to finish with them however the setting
  // is changed halfway through — otherwise the recorded condition is a lie and
  // the comparison is built on it.
  #specialists(run = null) {
    if (typeof run?.request?.specialists === "boolean") return run.request.specialists;
    return this.database.getSettings().intelligence?.agentSpecialists !== false;
  }
  ...
  async proposePlan({ objective, successCriteria, providerId, model, budgets, specialists = this.#specialists() }) {
    const tools = this.availableTools();
    const normalizedBudgets = normalizeGoalBudgets(budgets);
    const roster = this.#roster(specialists);
    const messages = [
  ...
  async create(body) {
    ...
    // Fixed here, once, and written onto the run. Every later decision reads it
    // back off the run rather than asking the setting again.
    const specialists = this.#specialists();
    const proposed = body.plan
      ? {
        plan: validateGoalPlan(body.plan, {
          availableTools: this.availableTools(), budgets: normalizeGoalBudgets(body.budgets), roster: this.#roster(specialists)
        }),
        route: { providerId, model, fallback: false },
        budgets: normalizeGoalBudgets(body.budgets)
      }
      : await this.proposePlan({ objective, successCriteria, providerId, model, budgets: body.budgets, specialists });
    providerId = proposed.route.providerId;
    model = proposed.route.model;
    // A goal raised from a message belongs to the conversation it was asked in.
    // Only a goal with no home of its own gets a conversation minted for it.
    let conversationId = String(body.conversationId || "");
    if (!conversationId) {
      conversationId = this.database.createConversation(`Agent · ${objective.slice(0, 70)}`).id;
      this.database.addMessage({ conversationId, role: "user", content: objective, status: "complete", metadata: { agentGoal: true, successCriteria } });
    }
    const run = this.agentRuntime.createGoalRun({
      conversationId, projectId: project.id, objective, successCriteria,
      providerId, modelId: model, budgets: proposed.budgets, plan: proposed.plan, availableTools: this.availableTools(),
      request: { provider: providerId, model, projectId: project.id, successCriteria, fallback: proposed.route.fallback, specialists }
    });
  ...
  // Read-only tools, by name. The escalation rule needs this to decide whether
  // a plan can start without asking; taken from the registry rather than a list
  // kept here, so a tool that gains a risk level cannot quietly stay auto-approved.
  #readOnlyTools() {
    return this.toolRegistry.list()
      .filter((tool) => tool.enabled && (tool.risk === "read" || tool.risk === "sandbox"))
      .map((tool) => tool.name);
  }

  // Would this message be better answered by a run than by a chat turn?
  //
  // Cheap and text-only: no model is called to find out, because the whole
  // point is that an ordinary question must not pay for this.
  wouldEscalate({ text, mode = "standard", images = [], projectId = "" } = {}) {
    const settings = this.database.getSettings();
    if (settings.toolsEnabled === false) return { escalate: false, reasons: ["tools are switched off"] };
    if (settings.intelligence?.autoGoals === false) return { escalate: false, reasons: ["multi-step answers are switched off"] };
    const project = this.projectService.get(projectId) || this.projectService.defaultProject();
    return shouldRunAsGoal({
      text, mode, images,
      toolsEnabled: true,
      hasProjectFolder: Boolean(project?.folderConnected),
      hasVault: this.vaultService.connected()
    });
  }

  // The whole of what /agent's form used to ask for, worked out instead.
  //
  // Criteria come from a small JSON call rather than from the person, because
  // "what would make this answer right" is a question about their own request
  // that they should not have to answer twice. If that call fails the request
  // itself becomes the single criterion — a worse gate than a written one, but
  // a real one, and better than refusing to run.
  async #deriveGoal(text, providerId, model) {
    const fallback = { objective: text.slice(0, 20_000), successCriteria: [`The request is answered from evidence gathered in this run: ${text.slice(0, 300)}`] };
    try {
      const { content } = await this.#complete(providerId, model, [
      ...
      const criteria = (Array.isArray(parsed.successCriteria) ? parsed.successCriteria : [])
        .map((item) => String(item).trim()).filter(Boolean).slice(0, 4);
      if (!criteria.length) return fallback;
      return { objective: String(parsed.objective || text).trim().slice(0, 20_000) || fallback.objective, successCriteria: criteria };
    } catch {
      return fallback;
    }
  }

  // A message becomes a run, or does not, and says which.
  //
  // Returns { started: false } for anything that should stay an ordinary chat
  // turn — including a plan that turned out not to be worth running, which is
  // only knowable after the planner has answered. Chat then answers normally
  // and the person never learns this happened, which is the intent: the
  // escalation is a decision Evolv makes, not a mode the person enters.
  async fromMessage({ conversationId, text, providerId, model, projectId = "", mode = "standard", images = [], budgets = null }) {
    const verdict = this.wouldEscalate({ text, mode, images, projectId });
    if (!verdict.escalate) return { started: false, ...verdict };
    const { objective, successCriteria } = await this.#deriveGoal(text, providerId, model);
    let run;
    try {
      run = await this.create({
        conversationId, objective, successCriteria, projectId,
        provider: providerId, model, budgets: budgets || { maxSteps: 8, maxRuntimeMs: 10 * 60 * 1000 }
      });
    } catch (error) {
      // Planning failed, so there is no run. Chat answers the message itself
      // rather than showing the person an error for something they did not ask
      // for.
      return { started: false, reasons: [`planning failed: ${error.message}`] };
    }
    const plan = run.plans?.find((item) => item.status === "active")?.definition
      || run.plans?.at(-1)?.definition;
    if (!planEarnsARun(plan)) {
      this.agentRuntime.cancel(run.id, "a single answer covers this");
      return { started: false, reasons: ["the plan was one step; a chat turn covers it"] };
    }
    // Reading is what chat already does when it answers a question, so a plan
    // that only reads starts on its own. A plan that proposes a change stops
    // here and shows itself, and the person approves it in the conversation.
    const readOnly = planOnlyReads(plan, { readOnlyTools: this.#readOnlyTools() });
    if (readOnly) await this.approve(run.id);
    return {
      started: true, run: this.agentRuntime.get(run.id), plan,
      needsApproval: !readOnly, reasons: verdict.reasons, successCriteria
    };
  }
  ...
  revise(runId, plan) {
    ...
    return this.agentRuntime.reviseGoalPlan(runId, plan, {
      availableTools: this.availableTools(), roster: this.#roster(this.#specialists(run))
    });
  }
  ...
  async #modelStep(run, step, signal) {
    ...
    const specialists = this.#specialists(run);
    const agent = resolveAgent(this.#plannedStep(run, step)?.agent, {
      step: { type: step.kind }, roster: this.#roster(specialists)
    });
    const system = agentSystemPrompt(agent, boundary);
    this.agentRuntime.addEvent(run.id, step.id, "agent.assigned", { agent: agent.id, name: agent.name, specialists });
    ...
    // Not in the control arm. A pinned model is part of what specialists buy
    // you, so letting one through here would put the two arms on different
    // models and the comparison would measure that instead.
    const override = specialists ? agentModelOverride(this.database.getSettings(), agent.id) : null;
    const candidates = [
      ...
      try {
        ...
        if (["analyze", "verification", "report"].includes(active.kind)) {
          ...
          const stepAgent = resolveAgent(this.#plannedStep(current, active)?.agent, {
            step: { type: active.kind }, roster: this.#roster(this.#specialists(current))
          });
